...
ServiceWorkerWare.prototype.onFetch = function sww_onFetch(evt) {
  ...
  // Push the fallback middleware at the end of the list if it's defined
  // also being sure that wont be executed if there is a response already.
  if (this.fallbackMw) {
    var fallback = this.fallbackMw;
  ...
  if (steps.length === 0) {
    // If no fallback mw, we go to the network.
    // XXX: We could setup a set of policies here too.
