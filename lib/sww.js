...
ServiceWorkerWare.prototype.TERMINATION_TOKEN = {};
...
ServiceWorkerWare.prototype.onFetch = function sww_onFetch(evt) {
  ...
  evt.respondWith(this.runTask(steps, 0, evt.request, null));
};

ServiceWorkerWare.prototype.runTask =
function (tasks, taskNumber, request, response) {
  if (taskNumber >= tasks.length) {
    return Promise.resolve(response);
  }

  var mw = tasks[taskNumber];
  var endWith = this.endWith.bind(this);
  var returnValue = this.normalizeMwAnswer(mw(request, response, endWith));
  return returnValue.then(function (value) {
    var nextRequest, nextResponse;
    var isArray = Array.isArray(value);

    if (isArray && value[0] === this.TERMINATION_TOKEN) {
      return Promise.resolve(value[1] || response);
    }
      ...
      nextRequest = value.length > 0 ? value[0] : nextRequest;
      nextResponse = value.length > 1 ? value[1] : nextResponse;
    }
      ...
      var error = 'Type error: middleware must return a Response or ' +
                  'Request or a Promise resolving to these types.'
      return Promise.reject(new Error(error));
    }

    return this.runTask(tasks, taskNumber + 1, nextRequest, nextResponse);
  }.bind(this));
};

ServiceWorkerWare.prototype.endWith = function (response) {
  return [this.TERMINATION_TOKEN, response];
};

ServiceWorkerWare.prototype.normalizeMwAnswer = function (answer) {
...
ServiceWorkerWare.decorators = {
  ...
  stopAfter: function (mw) {
    return [
      function (req, res, endWith) {
        var returned = mw(req, res, endWith);
      },
      function (req, res, endWith) {
        return endWith(res);
      }
