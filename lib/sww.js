...
ServiceWorkerWare.prototype.onFetch = function sww_onFetch(evt) {
  ...
  evt.respondWith(runTask(steps, 0, evt.request, null));

  function runTask(tasks, taskNumber, request, response) {
    if (taskNumber >= tasks.length) {
      return Promise.resolve(response);
    }

    var mw = tasks[taskNumber];
    var returnValue = normalize(mw(request, response, endWith));
    return returnValue.then(function (value) {
      var nextRequest, nextResponse;
      var isArray = Array.isArray(value);

      if (isArray && value[0] === terminationToken) {
      ...
      return runTask(tasks, taskNumber + 1, nextRequest, nextResponse);
    });
  }

  function endWith(response) {
    return [terminationToken, response];
  }
