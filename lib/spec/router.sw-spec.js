...
describe('Router instances', function () {
  ...
  describe('utility wrappers', function() {
    it('router.methods should contain the standard HTTP verbs and `all`',
    function() {
  ...
  describe('proxyMethods()', function() {
    ...
    describe('Object as handler', function() {
      ...
      it('should throw an error if the object does not have onFetch()',
      function() {
        var invalidHandler = {};
        router.proxyMethods(object);
        expect(function() { object.get('/', invalidHandler); })
          .to.throw(Error, 'This middleware cannot handle fetch request');
      });
    });
  });
  ...
  describe('Matching algorithm', function() {
    ...
    describe('match()', function() {
      ...
      it('should match multiple middlewares in the same order as they were ' +
      'added', function() {
        var mw2 = function () {};
        router.get('/', mw);
        router.get('/', mw2);
      ...
      it('should return an empty array if there is no middleware attached to ' +
      'this particular method', function() {
        router.get('/', mw);
        expect(router.match('post', '/')).to.be.empty;
      });

      it('should distinguish between middlewares attached to different ' +
      'methods', function() {
        var mw2 = function () {};
        router.get('/', mw);
        router.post('/', mw2);
      ...
      it('should distinguish between middlewares attached to different URL',
      function() {
        router.get('/a', mw);
        router.get('/b', mw);
      ...
      it('trailing placeholders (/foo/* etc.) should match arbitrary prefixes',
      function() {
        var mw2 = function () {};
      ...
      it('infix placeholders (/foo/*/bar etc.) should match arbitrary path ' +
      'chunks', function() {
        var mw2 = function () {};
      ...
      it('multiple placeholders (/foo/*/bar/* etc) should be allowed',
      function() {
        var mw2 = function () {},
            mw3 = function () {};
        ...
        expect(router.match('get', '/matched/suf/123/fix'))
          .to.deep.equal([mw2]);
        expect(router.match('get', '/any/other/suf/123/456/fix'))
          .to.deep.equal([mw2]);
        ...
        expect(router.match('get', '/inner/suf/other/fix'))
          .to.deep.equal([mw2, mw3]);
        expect(router.match('get', '/inner/suf/other/fix/'))
          .to.deep.equal([mw3]);

        expect(router.match('get', '/pre/123/fix/inner/suf/456/fix/'))
          .to.deep.equal([mw, mw3]);
        expect(router.match('get', '/pre/123/fix/inner/suf/456/fix'))
          .to.deep.equal([mw, mw2, mw3]);
        expect(router.match('get', '/pre/suf/inner/fix/'))
          .to.deep.equal([mw, mw3]);
      });

      it('named placeholders (:foo) must match at least one character ' +
      '(no empty matches allowed)', function() {
        router.get('/:foo', mw);
        expect(router.match('get', '/')).to.be.empty;
        expect(router.match('get', '/foo')).to.deep.equal([mw]);
        expect(router.match('get', '/foo/bar')).to.deep.equal([mw]);
      ...
      it('placeholder crowding for anonymous placeholder (**) should be ' +
      'treated same as a single placeholder', function() {
        var mw2 = function () {};
        ...
        expect(router.match('get', '/foo'))
          .to.deep.equal(router.match('get', '/foo'));
        expect(router.match('get', '/foobar'))
          .to.deep.equal(router.match('get', '/foobar'));
        expect(router.match('get', '/foobabarbar'))
          .to.deep.equal(router.match('get', '/foobabarbar'));
        expect(router.match('get', '/foo/bar/baz'))
          .to.deep.equal(router.match('get', '/foo/bar/baz'));
        expect(router.match('get', '/foo/doh/bar'))
          .to.deep.equal(router.match('get', '/foo/doh/bar'));
      });

      it('placeholder crowding for named placeholder (:foo:bar) should throw',
      function() {
        expect(function () { router.get('/foo/:bar:baz', mw); })
          .to.throw(Error, 'Invalid usage of named placeholders');
        expect(function () { router.get('/foo/:bar:baz:shoo', mw); })
          .to.throw(Error, 'Invalid usage of named placeholders');
      });

      it('mixed crowdings (*:foo, :bar** etc.) should throw', function() {
        expect(function () { router.get('/foo/*:bar', mw); })
          .to.throw(Error, 'Invalid usage of named placeholders');
        expect(function () { router.get('/:foo**/:bar', mw); })
          .to.throw(Error, 'Invalid usage of named placeholders');
        expect(function () { router.get('/foo/\\*:bar', mw); })
    ...
    describe('_parseSimplePath()', function() {
      it('on invalid path specification it should throw an error', function() {
        expect(function () { router._parseSimplePath(']['); })
          .to.throw(Error, 'Invalid path specified');
      });

      it('should support anonymous placeholders (*)', function() {
        expect(router._parseSimplePath('/a*b').regexp.source)
          .to.equal('\\/a(?:.*?)b$');
      });
      ...
      it('should support named placeholders (:foo)', function() {
        expect(router._parseSimplePath('/a/:foo/b').regexp.source)
          .to.equal('\\/a\\/(.+?)\\/b$');
      });
      ...
      it('should support multiple/mixed placeholders (*, :foo)', function() {
        expect(router._parseSimplePath('/a/*/*/b').regexp.source)
          .to.equal('\\/a\\/(?:.*?)\\/(?:.*?)\\/b$');
        expect(router._parseSimplePath('/a/:foo/:bar/b').regexp.source)
          .to.equal('\\/a\\/(.+?)\\/(.+?)\\/b$');
        expect(router._parseSimplePath('/a/*/:foo/b').regexp.source)
          .to.equal('\\/a\\/(?:.*?)\\/(.+?)\\/b$');
      });

      it('should support escaping of placeholder special characters (* and :)',
      function() {
        expect(router._parseSimplePath('/a\\*/*/b').regexp.source)
          .to.equal('\\/a\\*\\/(?:.*?)\\/b$');
        expect(router._parseSimplePath('/a/:foo\\:80/b').regexp.source)
          .to.equal('\\/a\\/(.+?):80\\/b$');
      });
    });
