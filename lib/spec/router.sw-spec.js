...
describe('Router instances', function () {
  ...
  describe('proxyMethods()', function() {
    ...
    // Currently failing, due to a testability issue: the handler gets binded,
    // which prevents to test the equality of the handlers.
    // If you guys have any idea on how to improve that.
    it('should support an object as a middleware', function() {
      var cb = sinon.spy(router, 'add');
      var handler = function() {};
      var handlerWithOnFetch = {
        onFetch: handler
      };
      router.proxyMethods(object);
      object.get('/', handlerWithOnFetch);
      var bindedHandler = handlerWithOnFetch.onFetch.bind(handlerWithOnFetch);
      expect(router.add.calledWith('get', '/', bindedHandler)).to.be.true;
    });
  ...
  describe('match()', function() {
    ...
    describe('"all" method', function() {
      ...
      // Currently failing, looks like a bug to me
